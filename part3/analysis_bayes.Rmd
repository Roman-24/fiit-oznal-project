...
#### Riešenie
```{r}
joint_prob <- cpquery(fitted_network, (warning_PM2.5 == "Warning" & warning_SO2 == "Warning"))
print(paste("Joint Probability of Warning for PM2.5 and SO2: ", joint_prob))
```
...
print(paste("Marginal Probability of Warning for NH3: ", marginal_prob_nh3))
```

### 3. Conditional Probability

**Otázka 1:** Aká je pravdepodobnosť, že dostaneme varovanie pre PM10, vzhľadom na to, že úrovne PM2,5 a CO vyvolali varovanie?

**Matematická reprezentácia:**

$$
P(\text{Warning}_{\text{PM10}} = \text{"Warning"} \mid \text{Warning}_{\text{PM2.5}} = \text{"Warning"}, \text{Warning}_{\text{CO}} = \text{"Warning"})
$$

#### Riešenie

```{r}
cond_prob_pm10 <- cpquery(fitted_network, event = (warning_PM10 == "Warning"),
                          evidence = (warning_PM2.5 == "Warning" & warning_CO == "Warning"))
print(paste("Conditional Probability of Warning for PM10 given Warnings for PM2.5 and CO: ", cond_prob_pm10))
```

**Otázka 2:** Aká je pravdepodobnosť, že dostaneme varovanie pre SO2, vzhľadom na to, že neexistuje žiadne varovanie pre NH3 a vysoká koncentrácia (varovanie) pre C2H3NO5?

**Matematická reprezentácia:**
$$
P(\text{Warning}_{\text{SO2}} = \text{"Warning"} \mid \text{Warning}_{\text{NH3}} = \text{"No Warning"}, \text{Warning}_{\text{C2H3NO5}} = \text{"Warning"})
$$

#### Riešenie
```{r}
cond_prob_so2 <- cpquery(fitted_network, event = (warning_SO2 == "Warning"),
                         evidence = (warning_NH3 == "No Warning" & warning_C2H3NO5 == "Warning"))
print(paste("Conditional Probability of Warning for SO2 given No Warning for NH3 and Warning for C2H3NO5: ", cond_prob_so2))
```
