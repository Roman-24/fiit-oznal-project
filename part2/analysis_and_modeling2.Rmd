...
Načítanie dát
```{r}
labor_measurements <- read.csv('/Users/roman.osadsky/Desktop/ROMAN/FIIT/OZNAL_PROJECT/oznal_project1/data/measurements.csv', sep = '\t')
labor_stations <- read.csv('/Users/roman.osadsky/Desktop/ROMAN/FIIT/OZNAL_PROJECT/oznal_project1/data/stations.csv', sep = '\t')
```
...
legend("bottomright", legend=c("GLM", "LDA", "SVM"), col=c("red", "blue", "green"), lwd=2)
...
```{r}
# Generating PR curves with the curve parameter set to TRUE
...
# Check the structure of the PR curve objects
str(pr_glm)
str(pr_lda)
str(pr_svm)
...
```{r}
library(caret)
# Extracting precision and recall from confusion matrices
